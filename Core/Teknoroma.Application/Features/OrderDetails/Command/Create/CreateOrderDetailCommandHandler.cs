	...
	public class CreateOrderDetailCommandHandler : IRequestHandler<CreateOrderDetailCommandRequest, Unit>
	{
        ...
        public async Task<Unit> Handle(CreateOrderDetailCommandRequest request, CancellationToken cancellationToken)
		{
			...
			foreach (var item in request.CartItems)
			{
				...
				//ProductStock
				var getProduct = await _mediator.Send(new GetByIdProductQueryRequest { ID = item.ID });
				getProduct.UnitsInStock -= item.Quantity;
				UpdateProductCommandRequest updateProductCommandRequest = _mapper.Map<UpdateProductCommandRequest>(getProduct);
