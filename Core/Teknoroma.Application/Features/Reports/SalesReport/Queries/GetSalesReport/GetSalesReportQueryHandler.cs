...
namespace Teknoroma.Application.Features.Reports.SalesReport.Queries.GetSalesReport
{
	public class GetSalesReportQueryHandler : IRequestHandler<GetSalesReportQueryRequest, GetSalesReportQueryResponse>
	{
		...
		public async Task<GetSalesReportQueryResponse> Handle(GetSalesReportQueryRequest request, CancellationToken cancellationToken)
		{
			...
			//Selling
			var bestSellingProduct = orders.SelectMany(order => order.OrderDetails)
			...
			var bestSellingCategory = orders.SelectMany(order => order.OrderDetails)
			...
			var bestSellingCustomer = orders
				...
				.Select(grouped => new
				{
					...
					TotalSales = grouped.Select(order => order.OrderDetails).SelectMany(details => details).Sum(detail => detail.Quantity)
				}).OrderByDescending(x => x.TotalSales).FirstOrDefault();
			...
			var bestSellingBranch = orders
				...
				.Select(grouped => new
				{
					...
					TotalSales = grouped.Select(order => order.OrderDetails).SelectMany(details => details)
			...
			//Earning
			var bestEarningProduct = orders.SelectMany(order => order.OrderDetails)
				.GroupBy(orderDetail => orderDetail.Product.ProductName)
				.Select(grouped => new
				{
					ProductName = grouped.Key,
			...
			var bestEarningCategory = orders.SelectMany(order => order.OrderDetails)
				.GroupBy(orderDetail => orderDetail.Product.Category.CategoryName)
				.Select(grouped => new
				{
					CategoryName = grouped.Key,
			...
			var bestEarningBranch = orders.GroupBy(x => x.Branch.BranchName)
				...
				{
					BranchName = grouped.Key,
					TotalPrice = grouped.Select(order => order.OrderDetails).SelectMany(details => details)
			...
			var bestEarningCustomer = orders.GroupBy(x=>x.Customer.FullName)
				...
				{
					FullName = grouped.Key,
					TotalEarning = grouped.Select(order => order.OrderDetails).SelectMany(details => details)
					.Sum(detail => detail.Quantity * detail.UnitPrice)
